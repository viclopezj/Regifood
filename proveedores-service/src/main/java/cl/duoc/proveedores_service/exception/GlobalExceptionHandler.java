...
public class GlobalExceptionHandler {
    ...
    // Configuración para nombre de proveedor duplicado
    @ExceptionHandler(NombreProveedorExistenteException.class)
    public ResponseEntity<?> errorNombreProveedorExistente(NombreProveedorExistenteException ex) {
        ErrorResponse response = new ErrorResponse(
                HttpStatus.BAD_REQUEST.value(),
                "Error en reglas de negocio (Nombre Proveedor)",
                ex.getMessage(),
                LocalDateTime.now()
        );
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    ...
    // Configuración para error de tipo
    @ExceptionHandler(TipoProveedorInvalidoException.class)
    public ResponseEntity<?> errorTipoInvalido(TipoProveedorInvalidoException ex) {
        ErrorResponse response = new ErrorResponse(
                HttpStatus.BAD_REQUEST.value(),
                "Error en reglas de negocio (Tipo)",
                ex.getMessage(),
                LocalDateTime.now()
        );
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
